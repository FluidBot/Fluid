package com.fluidbot.bot.script.randevent.impl;

import java.awt.Rectangle;

import com.fluidbot.bot.accounts.Account;
import com.fluidbot.bot.script.ScriptManifest;
import com.fluidbot.bot.script.api.tools.Game.GameState;
import com.fluidbot.bot.script.randevent.RandomEvent;
    ...
    public boolean init() {
        if (game.getGameState() == GameState.LOGIN) {
        	if (getContext().getAccount() == null || getContext().getAccount().getUsername().equals("null")
                ...
                log("You must have an account saved to use Auto Login. Make sure you press the save button. You can also try editing the Accounts.txt in your Fluid folder.");
                return false;
        	} else {
        		return true;
        	}
