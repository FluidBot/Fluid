...
import com.fluidbot.bot.IOHelper;
import com.fluidbot.bot.script.ScriptInfo;
import com.fluidbot.bot.util.ScriptClassLoader;
    ...
    public static List<byte[]> getScriptByteList() throws Exception {
        ...
        if (!data.isEmpty()) {
            for (final String[] array : data) {
                ...
                final URL url = new URL("http://www.fluid.com/repo/fetcher.php");
                final HttpURLConnection conn = (HttpURLConnection) url.openConnection();
                conn.setDoInput(true);
                conn.setDoOutput(true);
                conn.setRequestMethod("POST");
